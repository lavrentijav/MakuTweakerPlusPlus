...
std::vector<CpuSampleRow> MetricsDb::QueryCpu(int64_t sinceTs, int procIndex,
                                              int64_t untilTs) const {
    std::vector<CpuSampleRow> out;
    if (!db_) return out;
    sqlite3_stmt* stmt = nullptr;
    const char* sql = procIndex >= 0
                          ? (untilTs > 0
                                 ? "SELECT ts,proc_index,usage FROM cpu_samples WHERE ts>=? AND "
                                   "ts<=? AND proc_index=? ORDER BY ts"
                                 : "SELECT ts,proc_index,usage FROM cpu_samples WHERE ts>=? AND "
                                   "proc_index=? ORDER BY ts")
                          : (untilTs > 0
                                 ? "SELECT ts,proc_index,usage FROM cpu_samples WHERE ts>=? AND "
                                   "ts<=? ORDER BY ts,proc_index"
                                 : "SELECT ts,proc_index,usage FROM cpu_samples WHERE ts>=? ORDER "
                                   "BY ts,proc_index");
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) return out;
    int bind = 1;
