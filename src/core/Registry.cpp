...
bool EnsureHiveWrite(HKEY root) {
    if (!os::RequiresElevation(root)) return true;
    return os::EnsureAdmin(nullptr);
}

const char* HiveName(HKEY root) {
    if (root == HKEY_LOCAL_MACHINE) return "HKLM";
    if (root == HKEY_CURRENT_USER) return "HKCU";
    return "HK?";
}

void LogRegOp(const char* op, HKEY root, const std::wstring& subKey, const std::wstring& name,
              bool ok) {
    if (!logf::Active() || ok) return;
    logf::Warn(std::string(op) + " failed " + HiveName(root) + "\\" + util::ToUtf8(subKey) +
               "\\" + util::ToUtf8(name));
}
...
bool DeleteKeyTree(HKEY root, const std::wstring& subKey) {
    if (!EnsureHiveWrite(root)) return false;
...
bool SetDword(HKEY root, const std::wstring& subKey, const std::wstring& name, DWORD value) {
    ...
    if (!CreateKey(root, subKey)) {
        HKEY key{};
        if (!Open(root, subKey, KEY_SET_VALUE, key)) {
            LogRegOp("SetDword", root, subKey, name, false);
            return false;
        }
        ...
        if (!ok) LogRegOp("SetDword", root, subKey, name, ok);
        return ok;
    }
    HKEY key{};
    if (!Open(root, subKey, KEY_SET_VALUE, key)) {
