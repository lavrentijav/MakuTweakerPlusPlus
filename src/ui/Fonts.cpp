...
void InitFonts(HWND hwnd) {
    ...
    const wchar_t* uiCandidates[] = {L"SegoeUIVF.ttf", L"SegoeUIVariable.ttf", L"segoeui.ttf"};
    std::string segoeUi;
    for (auto f : uiCandidates) {
        const std::string p = SystemFontPath(f);
        if (FileExists(p)) {
            segoeUi = p;
            break;
    ...
    if (segoeUi.empty()) segoeUi = SystemFontPath(L"segoeui.ttf");
...
void ReloadFonts(HWND hwnd) {
    ...
    if (hwnd) {
        ...
        maku::app::Application::Instance().RefreshGlassPipeline();
    }
}
