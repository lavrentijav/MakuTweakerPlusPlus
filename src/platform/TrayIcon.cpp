...
bool TrayIcon::Create(HWND hwnd, HINSTANCE inst) {
    ...
    std::wstring iconPath = maku::util::GetExeDirectory() + L"\\assets\\icons\\MakuTweaker.ico";
    nid_.hIcon = static_cast<HICON>(LoadImageW(nullptr, iconPath.c_str(), IMAGE_ICON, 0, 0,
                                                LR_LOADFROMFILE | LR_DEFAULTSIZE));
    if (!nid_.hIcon)
        nid_.hIcon = LoadIconW(inst, MAKEINTRESOURCEW(0));
    wcscpy_s(nid_.szTip, L"MakuTweaker");
